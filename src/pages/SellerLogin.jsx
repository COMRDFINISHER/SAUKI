...
export default function SellerLogin() {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(({ data }) => {
      if (data.session) navigate("/seller/dashboard");
    });
  }, [navigate]);
  ...
  return (
    <div style={{ maxWidth: 380, margin: "0 auto", minHeight: "100vh", padding: "24px 20px" }}>
      ...
      <div style={{ textAlign: "center", marginTop: 16, fontSize: 12.5, color: "#888780" }}>
        New business owner?{" "}
        <Link to="/seller/signup" style={{ color: "#2C2C2A" }}>Register here</Link>
      </div>
    </div>
  );
}
